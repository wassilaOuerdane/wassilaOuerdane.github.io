...
5.1. Introduction to Argumentation Theory
==========================================
...
1- Constructing arguments in favor/against statements (pertaining to beliefs or decisions) (see section Modeling arguments), 
...
In the rest of this course, we will detail what do we mean by an argument, an attack relation and how the arguments are used within a negotiation process. 
...
In this course, we will adopt the premise-conclusion pairs as a structure of an argument, which is defined as follows: 
...
:math:`\rightarrow` *Who is the killer?* ...Wait, the answer is in a few lines :-) 


Let's go back to our Engine choice: Generating arguments: 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
At the core of the dialogue between the agents, the performative **ARGUE**.  It is used to explain to the other party in the dialogue
...
position. As a consequence, the content of such a message is of the form: :math:`B \leftarrow A`, such that the terms of :math:`A` aare the **premises** and :math:`B` a positive or negative literal, which is the conclusion. 
...
As it was discussed in section ??? (see the figure-right) antoher component in our ABN agent is the *argument selection*. The question is:
...
the ocunterparts' initial information and the payoffs they receive from the different outcomes (i.e. action profiles). This raises the opportunity to use **learning** techniques in order to find
patterns in the counterpart's behaviour an duse these findings in future encounters with the same (or similar) counterpart(s) (see for e.g. `Sandholm and Crites, 1995 <http://mas.cs.umass.edu/Documents/Sandholm/biosystems95.ps>`_). 
...
order* (you may choose another stratgy if you want!). For instance, Agent1 (see its perfomance table) has four positive arguments 
...
2- Update or test the interaction protocol to implement the situation above.  
...
3-  Update the interaction protocol to take into account the attack function and to implement the following situation: 
 ...
 Agent2 to Agent1:  reply with one of the previous counter-arguement --> ARGUE(...) 


4- In addition, at each ARGUE message, **store in a data structure the set of exchanged arguments**, such that it is possible to extract for each argument its set of counter-arguements.


4. Status of arguments? 
------------------------

The key question to ask about such an argumentation framework is whether a given argument, :math:`x \in \mathcal{A}`, should be *accepted*. Indeed, an argumentation frameworks encodes, through the attack relations, 
